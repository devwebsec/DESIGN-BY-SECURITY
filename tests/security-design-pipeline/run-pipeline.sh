...
CONTRACT_EVAL="$TEST_DIR/semantic/validate-contract.py"
...
for artifact in intent architecture assets data_flows trust_boundaries threat_model attack_paths security_requirements controls validation dynamic_analysis fuzzing secure_build evidence security_gate operational_handoff lessons_learned redesign; do check_contains "$MANIFEST" "  - $artifact" "manifest required artifact: $artifact"; done
for hard_fail in critical_threat_without_requirement requirement_without_validation critical_attack_path_without_detection_or_explicit_gap unresolved_critical_design_flaw unknown_presented_as_evidence destructive_action_without_human_approval incident_closed_without_root_cause_feedback dynamic_analysis_processing_error_presented_as_pass required_retest_missing fuzzing_crash_without_analysis secure_build_provenance_missing secure_build_artifact_digest_missing; do check_contains "$MANIFEST" "  - $hard_fail" "manifest hard-fail: $hard_fail"; done
...
printf '\n--- Deterministic GOST 5.11/5.12 contract validation ---\n'
if python3 "$CONTRACT_EVAL" "$SEMANTIC_VALID"; then good 'GOST 5.11/5.12 security-design contract'; else bad 'GOST 5.11/5.12 security-design contract'; fi
...
printf '\n--- Level 8 GOST evidence validation ---\n'
if bash "$ROOT/gost-compliance/pipeline/gost-validate.sh"; then good 'Level 8 GOST evidence contract'; else bad 'Level 8 GOST evidence contract'; fi
